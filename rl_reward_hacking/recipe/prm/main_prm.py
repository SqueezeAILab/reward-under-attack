...
from .prm_ray_trainer import RayPRMPPOTrainer



@hydra.main(config_path="config", config_name="prm", version_base=None)
def main(config):
    ...
    def run(self, config):
        ...
        if config.reward_model.enable:
            if config.reward_model.strategy in {"fsdp", "fsdp2"}:
                from .fsdp_reward_model_worker import PRMRewardModelWorker
            else:
                raise NotImplementedError
            role_worker_mapping[Role.RewardModel] = ray.remote(PRMRewardModelWorker)
        ...
        if config.data.get("max_train_samples") is not None:
            ...
            if len(train_dataset) > max_train_samples:
                ...
                print(f"Limited training dataset to {max_train_samples} samples (original: {len(train_dataset.dataset)})")
        
        # Limit validation dataset size if specified
        if config.data.get("max_val_samples") is not None:
            ...
            if len(val_dataset) > max_val_samples:
                ...
                print(f"Limited validation dataset to {max_val_samples} samples (original: {len(val_dataset.dataset)})")
        
        train_sampler = create_rl_sampler(config.data, train_dataset)

        # Initialize the PPO trainer.
        trainer = RayPRMPPOTrainer(
            config=config,
            tokenizer=tokenizer,
            processor=processor,
